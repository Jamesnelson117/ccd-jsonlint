 ...
 * @returns boolean
 */
const filterListByExtension = (file, ext = 'json') => {
...
const lint = dir => {
  ...
  let files = getFileListFromDirectory(dir)
    .filter(file => filterListByExtension(file, 'json'));
  ...
  files.map(file => {
    ...
    if ((passed+failed) == files.length) {
      ...
      console.log(`   ${cross} ${failed} valid files`);
      console.log('\n');
      process.exit(failed);
    };
  });
